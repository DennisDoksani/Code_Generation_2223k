...
public class TransactionService {
  ...
  public List<TransactionResponseDTO> getTransactionsWithFilters(Pageable pageable, String ibanFrom,
      String ibanTo, Double amountMin, Double amountMax, LocalDate dateBefore,
      LocalDate dateAfter) {
    List<Transaction> transactions = transactionRepository.getTransactionsWithFilters(pageable,
        ibanFrom, ibanTo, amountMin, amountMax, dateBefore, dateAfter).getContent();
  ...
  public List<TransactionResponseDTO> getTransactionsOffAccount(Pageable pageable, String iban) {
    List<Transaction> transactions = transactionRepository.getTransactionsOffAccount(pageable, iban)
        .getContent();
  ...
  public TransactionResponseDTO addTransaction(TransactionDTO transactionDTO, User userPerforming) {
    Transaction newTransaction = transactionRepository.save(
        mapDtoToTransaction(transactionDTO, userPerforming));

    return mapTransactionToDto(newTransaction);
  }

  public Double getSumOfMoneyTransferred(String user, LocalDate date) {
    return transactionRepository.getSumOfMoneyTransferred(user, date, AccountType.CURRENT)
  ...
  public Boolean validTransaction(TransactionDTO dto) {
    Account accountTo = accountRepository.findById(dto.accountTo())
        .orElseThrow(
            () -> new EntityNotFoundException("Account " + dto.accountTo() + " not found"));
    Account accountFrom = accountRepository.findById(dto.accountFrom())
        .orElseThrow(
            () -> new EntityNotFoundException("Account " + dto.accountFrom() + " not found"));
    ...
    //This statement checks if money is being transferred to or from a savings account that does not belong to the same user
      if (((accountFrom.getAccountType() == AccountType.CURRENT
          && accountTo.getAccountType() == AccountType.SAVINGS) || (
          accountFrom.getAccountType() == AccountType.SAVINGS
              && accountTo.getAccountType() == AccountType.CURRENT))
          && accountFrom.getCustomer().getBsn() != accountTo.getCustomer().getBsn()) {
          throw new IllegalArgumentException(
              "You can not transfer money from or to a savings account that does not belong to the same user");
      }

      if (accountFrom.getBalance() - dto.amount() < accountFrom.getAbsoluteLimit()) {
          throw new IllegalArgumentException(
              "The amount you are trying to transfer will result in a balance lower than the absolute limit for this account");
      ...
      if (dto.amount() > accountFrom.getCustomer().getTransactionLimit()) {
          throw new IllegalArgumentException(
              "The amount you are trying to transfer exceeds the transaction limit");
      }

    // Checks first if the accountFrom has transactions made today and then if the amount will exceed the day limit
      if (accountTo.getAccountType() != AccountType.SAVINGS &&
          dto.amount() + getSumOfMoneyTransferred(accountFrom.getCustomer().getEmail(),
              LocalDate.now()) > accountFrom.getCustomer().getDayLimit()) {
          throw new IllegalArgumentException(
              "The amount you are trying to transfer will exceed the day limit for this account");
      }
  ...
  @Transactional
  // making the method transactional so that if any error occurs, the transaction will be rolled back
  public TransactionResponseDTO atmWithdraw(ATMWithdrawDTO withdrawDTO,
      String userPerformingEmail) {
